...
def main():
    ...
    parser.add_argument('--checkpoint', '-c', type=str, required=True, help='Path to checkpoint (.pt)')
    parser.add_argument('--task', type=str, default='go2')
    parser.add_argument('--num_envs', type=int, default=16)
    parser.add_argument('--hidden_dims', type=str, default='512,512,512', help='Comma-separated hidden dims')
    parser.add_argument('--sim_device', type=str, default='cuda:0', help='Simulation device, e.g. cpu or cuda:0')
    parser.add_argument('--rl_device', type=str, default=model_device, help='RL device for tensors (default from models.py)')
    parser.add_argument('--steps', type=int, default=0, help='Number of environment steps to run (0 -> uses 5 episodes length)')
    parser.add_argument('--render', action='store_true', help='Enable rendering (opens a viewer window)')
    ...
    rl_device = torch.device(args.rl_device if args.rl_device.startswith('cpu') or args.rl_device.startswith('cuda') else args.rl_device)
    action_high = torch.ones(action_dim, device=rl_device)

    print(f"Env created: task={args.task}, num_envs={args.num_envs}, obs_dim={obs_dim}, action_dim={action_dim}")
    ...
    try:
        ...
        print('Failed to load actor state_dict. This usually means the network architecture (hidden dims) does not match the checkpoint.')
        print('Error:', e)
    ...
    for step in range(total_steps):
        ...
        if len(done_ids) > 0:
            for did in done_ids.tolist():
                print(f"Episode finished (env={did}) length={int(episode_lengths[did].item())} reward={float(episode_rewards[did].item()):.3f}")
