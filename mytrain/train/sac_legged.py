    ...
    for step in track(range(max_iterations), description="训练中"):
        ...
        if step >= start_steps and len(rb) >= batch_size:
            for train_idx in range(train_times_per_collect):
                ...
                # 更新 Alpha
                alpha_loss = -(
                    log_alpha.exp() * (log_prob_pi + target_entropy).detach()
                ).mean()
            ...
            if step % 100 == 0:
                ...
                writer.add_scalar(
                    "Q/q_diff", (curr_q1 - curr_q2).abs().mean().item(), step
                )
                ...
                writer.add_scalar(
                    "Action/abs_mean", actions_pi.abs().mean().item(), step
                )
        ...
        if step % 1000 == 0 and completed_episodes > 0:
            ...
            print(
                f"Step {step} | Episodes: {completed_episodes} | Mean Reward: {mean_reward:.2f}"
            )
            completed_episodes = 0
            total_reward_sum = 0.0
